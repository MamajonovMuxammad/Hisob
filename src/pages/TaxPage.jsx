...
export default function TaxPage() {
  const [mode, setMode] = useState('usn')
  ...
  const calcTax = () => {
    ...
    if (mode === 'usn') {
      ...
      setResult({
        ...
        note: 'Срок оплаты: до 15 числа следующего месяца',
      })
    } else {
      const nds    = r * 0.12
      const profit = r * 0.25
      ...
      setResult({
        ...
        note: 'Квартальная отчётность по НДС и налогу на прибыль',
      })
    }
  }
    ...
    <div style={{ padding: '24px 28px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
        ...
        <div className="card" style={{ padding: 24 }}>
          ...
          <div className="toggle-group" style={{ marginBottom: 18, width: '100%' }}>
            {[['usn', 'ИП / УСН'], ['osn', 'ООО / ОСН']].map(([v, l]) => (
              <button key={v} className={`toggle-btn${mode === v ? ' active' : ''}`}
                style={{ flex: 1 }}
                onClick={() => { setMode(v); setResult(null) }}>{l}</button>
            ))}
          </div>
        ...
        <div className="card" style={{ padding: 24 }}>
          ...
          <div style={{ fontSize: 13, color: '#A0AEC0', lineHeight: 1.8 }}>
            <p style={{ fontWeight: 600, color: '#E2E8F0', marginBottom: 8 }}>📚 Справка по ставкам (2024)</p>
            {[
              ['НДФЛ', '12% — удерживается с зарплаты сотрудника'],
              ['ИНПС', '4% — удерживается, 1% — работодатель'],
